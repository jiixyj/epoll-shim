...
typedef void (*pollable_node_poll_t)(void *pollable_node, int kq, /**/
    uint32_t *revents);
...
static inline void
pollable_node_poll(PollableNode pollable_node, int kq, uint32_t *revents)
{
	assert(pollable_node.ptr != NULL);
	assert(pollable_node.vtable != NULL);
	assert(pollable_node.vtable->poll_fun != NULL);

	pollable_node.vtable->poll_fun(pollable_node.ptr, kq, revents);
}
